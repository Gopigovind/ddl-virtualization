  ...
  var SfDropDownList = function () {
    ...
    SfDropDownList.prototype.scrollListener = function (scrollArgs) {
      ...
      if (this.preStartIndex !== pStartIndex) {
        this.currentPage = viewInfo.currentPage;
          setTimeout(function () {
              var liHieght = _this.getRowHeight();
          _this.dotNetRef.invokeMethodAsync("VirtualRefresh", {
              virtualStartIndex: viewInfo.startIndex ? viewInfo.startIndex : 0,
              virtualEndIndex: viewInfo.endIndex ? viewInfo.endIndex : 0,
              liHeight: liHieght ? liHieght : 0,
              scrollTop: viewInfo.offsets && viewInfo.offsets.top ? viewInfo.offsets.top : 0
          }).then(function () {
      ...
      this.prevInfo = viewInfo; //this.blazorActiveKey = '';
      };
    ...
    SfDropDownList.prototype.getInfoFromView = function (direction, info, e) {
      ...
      if (infoType.direction === 'down') {
        ...
        if (sf.base.isNullOrUndefined(infoType.startIndex) || exactEndIndex > infoType.startIndex + Math.round(this.options.itemsCount / 2 + pageSizeBy4) && infoType.endIndex !== totalCount) {
          ...
          infoType.currentPage = Math.ceil(infoType.endIndex / this.options.itemsCount); //this.nextRowToNavigate = Math.floor(exactEndIndex - 1);
        }
        ...
        if (infoType.startIndex && infoType.endIndex) {
          ...
          if (exactTopIndex < loadAtIndex) {
            ...
            infoType.currentPage = Math.ceil(infoType.startIndex / this.options.itemsCount); //this.nextRowToNavigate = Math.ceil(exactTopIndex + 1);
          }
        }
    ...
    SfDropDownList.prototype.virtualScrollHandler = function (callback) {
      ...
      return function (e) {
        ...
        var check = _this.check(direction); // let isMaskRow: boolean = this.popupContentEle.querySelectorAll('.e-masked-row').length && current.axis != 'X';
        // if (current.entered || ((isNullOrUndefined(this.blazorActiveKey) || this.blazorActiveKey == '') && !this.fromWheel && isMaskRow)) {
        //     onEnterCallback(this.element, current, isLightScroll, direction, { top: top, left: left }, this.fromWheel, check);
        // }
    ...
    SfDropDownList.prototype.renderPopup = function (popupElement, popupHolderEle, openEventArgs, isModifiedPopup, options, dataItem) {
      ...
      var firstList = popupElement.querySelectorAll('li');
      ...
      if (sf.base.Browser.isDevice) {
        ...
        if (this.options.enableVirtualization) {
            ...
            setTimeout(function () {
                if (_this.options.scrollTop && (_this.options.value && !_this.options.isTyped)) {
    ...
    SfDropDownList.prototype.popupCreation = function (popupElement, offsetValue, left) {
      ...
      this.popupObj = new sf.popups.Popup(this.popupContainer, {
        ...
        close: function close() {
          ...
          if (_this.options.value == null || _this.options.isTyped) {
    ...
    initialize: function initialize(containerElement, element, dotnetRef, options) {
      if (element) {
        ...
        if (options.enableVirtualization) {
            return { 'itemSize': element.blazor__instance.getListHeight(), 'fullPopupHeight': window.outerHeight };
        }
      }
        return 0;
    },
